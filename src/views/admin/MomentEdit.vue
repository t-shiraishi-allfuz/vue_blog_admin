<template>
	<v-container v-if="momentDetail.id">
		<v-card>
			<v-card-title class="d-flex align-center">
				<div>
					<div class="text-h5">モーメント編集</div>
...
const thumbnailRules = [
	(v: File[] | null): boolean | string => {
		...
		if (momentDetail.value.thumbUrl) return true
...
const fetchMomentDetail = async () => {
	const momentId = route.query.id as string
	if (!momentId) {
		loading.value = false
		return
	}
...
const submitImages = async () => {
	...
	try {
		...
		console.log(momentDetail.value.thumbUrl)
...
const saveMoment = async () => {
	...
	try {
		...
		console.error('モーメント更新エラー:', error)
		Swal.fire({
			title: 'エラー',
			text: 'モーメントの更新に失敗しました',
			icon: 'error'
		})
	} finally {
		loading.value = false
